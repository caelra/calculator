...
export function useCalculator() {
  ...
  const evaluate = useCallback(
    async (operation: Operation, a: number, b?: number): Promise<number | null> => {
      ...
      try {
        const result =
          b === undefined ? await calculate(operation, a) : await calculate(operation, a, b)
  ...
  const pressOperator = useCallback(
    ...
    [state.loading, state.pendingOp, state.previous, state.overwrite, state.display, evaluate],
  )

  const pressEquals = useCallback(async () => {
    if (state.loading || state.pendingOp === null || state.previous === null) return
    await evaluate(state.pendingOp, state.previous, parseFloat(state.display))
  }, [state.loading, state.pendingOp, state.previous, state.display, evaluate])

  const pressSqrt = useCallback(async () => {
    if (state.loading) return
    await evaluate('sqrt', parseFloat(state.display))
  }, [state.loading, state.display, evaluate])
